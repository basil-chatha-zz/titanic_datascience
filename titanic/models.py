...
class MajorityVoteClassifier:
    ...
    def predict(self, X):
        if self.majority_vote is None:
            raise ValueError(
                "The majority vote classifier has to be trained before making predictions")
...
def run_majority_vote(X_train, X_test, y_train, y_test):
    ...
    logging.info('The prediction accuracy with the majority vote classifier is {:.1f}%'.format(
        accuracy * 100))
...
def run_logistic_regression(X_train, X_test, y_train, y_test):
    ...
    logging.info('The prediction accuracy with the ridge logistic regression classifier is {:.1f}%'.format(
        accuracy * 100))
